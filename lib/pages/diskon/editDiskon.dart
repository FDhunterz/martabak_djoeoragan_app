...
GlobalKey<ScaffoldState> _scaffoldKeyEditDiskon;
...
class EditDiskon extends StatefulWidget {
  ...
  @override
  _EditDiskonState createState() => _EditDiskonState();
}

class _EditDiskonState extends State<EditDiskon> {
  @override
  void initState() {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        _namaDiskonController =
            TextEditingController(text: responseJson['data1']['d_nama']);
        _awalDiskonController = TextEditingController(
          text: DateFormat('dd/MM/yyyy').format(
            DateTime.parse(
              responseJson['data1']['d_mulai'],
            ),
          ),
        );
        _initialAwalDiskon = DateTime.parse(responseJson['data1']['d_mulai']);
        _akhirDiskonController = TextEditingController(
          text: DateFormat('dd/MM/yyyy').format(
            DateTime.parse(
              responseJson['data1']['d_akhir'],
            ),
          ),
        );
        _initialAkhirDiskon = DateTime.parse(responseJson['data1']['d_akhir']);
        _catatanController = TextEditingController(
          text: responseJson['data1']['d_catatan'] == null
              ? ''
              : responseJson['data1']['d_catatan'],
        );

        if (responseJson['data1']['d_nominal'] == null) {
          _isNilai = false;
        } else {
          _isNilai = true;
        }
        _diskonNilaiController = MoneyMaskedTextController(
          thousandSeparator: ',',
          decimalSeparator: '.',
          initialValue: responseJson['data1']['d_nominal'] == null
              ? 0.00
              : double.parse(responseJson['data1']['d_nominal'].toString()),
        );

        _kategoriTotalPembelianController = MoneyMaskedTextController(
          initialValue: responseJson['data1']['d_kategori_harga'] == null
              ? 10000.00
              : double.parse(
                  responseJson['data1']['d_kategori_harga'].toString()),
          thousandSeparator: ',',
          decimalSeparator: '.',
        );

        _maxDiskonController = MoneyMaskedTextController(
          initialValue: responseJson['data1']['d_max_diskon'] == null
              ? 0.00
              : double.parse(responseJson['data1']['d_max_diskon'].toString()),
          thousandSeparator: ',',
          decimalSeparator: '.',
        );
        _dGabung = int.parse(responseJson['data1']['d_isdouble']);
        _dKonfirmasi = int.parse(responseJson['data1']['d_isconfirm']);
        _selectedlistDTipe = responseJson['data1']['d_type'] == 'ITEM'
            ? DTipe(
                id: 'ITEM',
                text: 'Potongan harga item',
              )
        ...
        _selectedDiskon = responseJson['data1']['d_persen'] == null
            ? 10
            : responseJson['data1']['d_persen'];
        ...
        _selectedProduk = responseJson['data1']['d_item'] == null
            ? Produk(
                id: 'all',
                nama: 'Semua Item',
              )
            : Produk(
                id: responseJson['data1']['d_item'].toString(),
                nama: responseJson['data1']['i_nama'],
              );
        setState(() {
          _isError = false;
          _isLoading = false;
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      key: _scaffoldKeyEditDiskon,
      appBar: AppBar(
        title: Text('Form Edit Data Diskon'),
