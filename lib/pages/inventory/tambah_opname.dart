...
class _TambahOpname extends State<TambahOpname> {
        ...
        for (var j = 0; j < listproduksementara.length; j++) {
          if (listproduksementara[j].qty < int.parse(uqty.text)) {
            if (int.parse(uqty.text) < 100) {
              buttonwidth = 84;
            } else if (int.parse(uqty.text) > 99 &&
                int.parse(uqty.text) < 1000) {
              buttonwidth = 99;
            } else if (int.parse(uqty.text) > 999 &&
                int.parse(uqty.text) < 10000) {
              buttonwidth = 110;
            } else if (int.parse(uqty.text) > 9999 &&
                int.parse(uqty.text) < 100000) {
              buttonwidth = 120;
            } else if (int.parse(uqty.text) > 99999 &&
                int.parse(uqty.text) < 1000000) {
              buttonwidth = 130;
            } else {
              buttonwidth = 150;
            }
  ...
  tambahsementara(code) {
    ...
    for (var i = 0; i < listprodukawal.length; i++) {
      ...
      } else if (listprodukawal[i].code == code &&
          listprodukawal[i].checked == false) {
                ...
                Container(
                  child: Column(
                    children: textsearch != null && textsearch != ''
                        ? listprodukawal
                            .where((ProdukAwal f) => f.nama
                                .toLowerCase()
                                .contains(textsearch.toLowerCase()))
                            .map((ProdukAwal f) => InkWell(
                                  onTap: () {
                                    if (f.checked == true) {
                                      setState(() {
                                        f.checked = false;
                        ...
                        Container(
                          ...
                          child: Container(
                              decoration: BoxDecoration(
                                color: Colors.white,
                                border: listproduksementara.length == 0
                                    ? null
                                    : Border(
                                        top: BorderSide(
                                            width: 1, color: Color(0xfffbaf18)),
                                        left: BorderSide(
                                            width: 10,
                                            color: Color(0xfffbaf18)),
                                        right: BorderSide(
                              ...
                              child: Column(
                                children: listproduksementara.length == 0
                                    ? <Widget>[
                                        Center(
                                          child: Column(
                                            crossAxisAlignment:
                                    ...
                                    : listproduksementara
                                        .map(
                                          (ProdukSementara f) => ListTile(
                                            onLongPress: () => popupbawah(
                                                UbahQty(
                                                    qty: f.qty, code: f.code)),
                                            title: Text(
                                                f.nama != null ? f.nama : ''),
                                            subtitle: Text(f.deskripsi != null
                                                ? f.deskripsi
                                                : ''),
                                            trailing: Container(
  ...
  Future<bool> onWillPop() async {
    ...
    if (currentBackPressTime == null ||
      ...
      if (listproduksementara.length != 0) {
        ...
        Fluttertoast.showToast(msg: "double click untuk kembali");
      }
      return false;
    }
    if (listproduksementara.length != 0) {
